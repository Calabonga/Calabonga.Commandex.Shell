...
public class DefaultGroupBuilder : IGroupBuilder
{
    ...
    [
        new() { Name = "HttpClient", Tags = ["http", "result"], Description = "Commands with HttpClient using for request to remote services (API)" },
        new() { Name = "Empty commands type", Tags = ["empty"], Description = "Commands that is not result returned to Shell" },
        new() { Name = "Wizard commands type", Tags = ["wizard", "dialog"], Description = "Commands that in modal dialog shows a few steps in the wizard." },
        new() { Name = "Commands with result", Tags = ["result"], Description = "Commands with some result for the Shell" },
        new() { Name = "Dialog", Tags = ["dialog"], Description = "Commands that run into modal dialog window of the Shell" },
        new() { Name = "Command for Person", Tags = ["person"], Description = "Command for Person entity management" },
        new() { Name = "Command with Parameter sharing", Tags = ["parameter"], Description = "Command for Person entity management" }
    ];
}